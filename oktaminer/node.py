...
class Miner(BasePollerFT):
    def configure(self):
        ...
        self.multiple_indicator_types = self.config.get(
            "multiple_indicator_types", False
        )
...
class UserMiner(BasePollerFT):
    def configure(self):
        ...
        self.query = self.config.get(
            "query", 'outcome.result eq "SUCCESS" and eventType eq "user.session.start"'
        )
...
class UserDetailMiner(BasePollerFT):
    def configure(self):
        """
        Example URI:
            https://<company>.okta.com/api/v1/logs?since=2021-02-24T00:00:00.000Z&filter=eventType eq "user.session.start"
        ...
        configurations:
            ...
            api_key (str): API Key for Okta
        """
        super(UserDetailMiner, self).configure()
    ...
    def _process_item(self, item):
        ...
        value = {
            ...
            "country": item['client'].get('geographicalContext', {'country': 'N/A'})['country'],
            "state": item['client'].get('geographicalContext', {'state': 'N/A'})['state'],
        }
        return [[indicator, value]]
